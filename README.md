...
3. **Validation** — the same rules (name required, message ≥ 5 chars, file required) are enforced in two places: client-side in the React form for fast inline feedback (with `aria-invalid`/`role="alert"`), and again on the Express side (`backend/src/validation.ts`) so the server never trusts client input. If the server still returns field errors (e.g. a file that passed client checks but exceeded the multer size limit), they're merged back into the form's field errors.
